...
public class TimeLogSystem {
    ...
    private static void afficherInformationsUtilisateur(Employe employe, String statut) {
        ...
        System.out.println("Bienvenue, " + employe.getNom() + ", Vous etes actuellement connecte au Projet  ");
...
public class TimeLogSystem {
    ...
    private static Discipline activiteCourante = null; // Pour suivre l'activité courante

    private static void debuterActivite(Employe employe, Scanner scanner) {
        // Vérifier si l'employé a déjà une activité en cours
        for (Discipline activite : disciplines) {
            if (activite.getEmploye() != null && activite.getEmploye().getId() == employe.getId() && activite.getFin() == null) {
                System.out.println("Vous avez déjà une activité en cours (" + activite.getNom() + "). Veuillez la terminer avant d'en commencer une nouvelle.");
                return; // Empêche de débuter une nouvelle activité
            }
        }
        ...
        activiteCourante = activite; // Met à jour l'activité courante
    ...
    private static void initialiserDonneesParDefaut() {
        ...
        employes.add(new Employe("Jamal", 1, LocalDate.now(), "123-45-6789", "Administrateur", Arrays.asList(1, 2)));
        employes.add(new Employe("Yannick", 2, LocalDate.now(), "234-56-7890", "Developpeur", Arrays.asList(3, 4)));
        employes.add(new Employe("Cedric", 3, LocalDate.now(), "345-67-8901", "Comptable", Arrays.asList(5, 6)));
    ...
    private static void afficherInformationsUtilisateur(Employe employe, String statut) {
        ...
        if (activiteCourante != null && activiteCourante.getEmploye().getId() == employe.getId()) {
            Projet projet = activiteCourante.getProjet();
            System.out.println("Bienvenue, " + employe.getNom() + ", Vous êtes actuellement connecté au Projet: " + projet.getNom());
        } else {
            System.out.println("Vous n'êtes actuellement associé à aucun projet.");
        }
    }
